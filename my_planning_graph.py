...
class ActionLayer(BaseActionLayer):
    ...
    def _inconsistent_effects(self, actionA, actionB):
        ...
        """"""
        effectlistA = self.children[actionA]
        effectlistB = self.children[actionB]

        for effectA in effectlistA:
            if ~effectA in effectlistB:
                return True
        for effectB in effectlistB:
            if ~effectB in effectlistA:
                return True
        return False
    ...
    def _interference(self, actionA, actionB):
        ...
        for effectA in effectlistA:
            for effectB in preconditionsB:
                if effectA == ~effectB:
                    return True
        for effectA in effectlistB:
            for effectB in preconditionsA:
                if effectA == ~effectB:
                    return True
        return False
    ...
    def _competing_needs(self, actionA, actionB):
        ...
        for parentA in preconditionsA:
            for parentB in preconditionsB:
                if self.parent_layer.is_mutex(parentA, parentB):
                    return True
                if self.parent_layer.is_mutex(parentB, parentA):
                    return True
        return False
...
class LiteralLayer(BaseLiteralLayer):
    ...
    def _inconsistent_support(self, literalA, literalB):
        ...
        layers.BaseLayer.parent_layer
        """
        actionlistA = self.parents[literalA]
        actionlistB = self.parents[literalB]

        for actionA in actionlistA:
            for actionB in actionlistB:
                if self.parent_layer.is_mutex(actionA, actionB) & self.parent_layer.is_mutex(actionB, actionA):
                    continue
                else:
                    return False
        return True
...
class PlanningGraph:
    ...
    def LevelCost(self, graph, goal):
        for i, layer in enumerate(graph.literal_layers, start=0):
            if goal in layer:
                return i
    ...
    def h_levelsum(self):
        ...
        for goal in self.goal:
            costs.append(self.LevelCost(self, goal))
        return sum(costs)
    ...
    def h_setlevel(self):
        ...
        while not self._is_leveled:
            ...
            if all_goals_met(layer) and goalsAreMutex(layer):
                return graph_level
            self._extend()
            graph_level += 1
